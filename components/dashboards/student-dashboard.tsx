...
import {
  CalendarCheck, BookOpen, GraduationCap, Bus, Clock, MapPin, ChevronRight,
} from 'lucide-react';
import {
    ...
    <div className="animate-fade-in">
      ...
      <div className="space-y-4 p-4 md:p-6">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <KpiCard label="My Attendance" value={`${attendancePct}%`} change={`${presentCount}/${myAttendance.length} days present`} trend={attendancePct >= 90 ? 'up' : 'down'} icon={CalendarCheck} iconColor="text-violet-600" iconBg="bg-violet-50" href="/attendance?tab=students" />
          <KpiCard label="Homework Due" value={String(pendingHomework.length)} change={`of ${myHomework.length} assigned`} icon={BookOpen} iconColor="text-pink-600" iconBg="bg-pink-50" href="/communication?tab=homework" />
          <KpiCard label="Upcoming Exams" value="1" change="Unit Test 1 · Sep 20" icon={GraduationCap} iconColor="text-orange-600" iconBg="bg-orange-50" href="/exams?tab=marks" />
          <KpiCard label="Bus Status" value={myRoute ? 'On Time' : 'N/A'} change={myRoute ? `${myRoute.progress}% on route` : 'Not assigned'} trend={myRoute ? 'up' : 'neutral'} icon={Bus} iconColor="text-teal-600" iconBg="bg-teal-50" href="/transport?tab=live-map" />
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/timetable?view=class" className="flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-slate-900 group">
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-slate-800">My Homework</h3>
            ...
            <div className="space-y-2">
              {myHomework.map(hw => (
                <Link key={hw.id} href="/communication?tab=homework">
                  <div className="flex items-start gap-3 rounded-lg border border-slate-100 p-3 group transition-colors hover:bg-slate-50">
                    <div className={cn(
