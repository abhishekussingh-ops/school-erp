    ...
    <div className="animate-fade-in">
      ...
      <div className="space-y-4 p-4 md:p-6">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
          <KpiCard label="Attendance" value={`${attendancePct}%`} change={presentCount === childAttendance.length ? 'Perfect!' : `${childAttendance.length - presentCount} absences`} trend={attendancePct >= 90 ? 'up' : 'down'} icon={CalendarCheck} iconColor="text-violet-600" iconBg="bg-violet-50" />
          <KpiCard label="Fee Due" value={`₹${dueAmount.toLocaleString('en-IN')}`} change={dueAmount > 0 ? 'Action needed' : 'All paid'} trend={dueAmount > 0 ? 'down' : 'up'} icon={CreditCard} iconColor="text-emerald-600" iconBg="bg-emerald-50" />
          <KpiCard label="Homework Due" value={String(childHomework.filter(h => !h.submissions.submitted).length)} change={`of ${childHomework.length} assigned`} icon={BookOpen} iconColor="text-pink-600" iconBg="bg-pink-50" />
          <KpiCard label="Upcoming Exams" value="1" change="Unit Test 1" icon={GraduationCap} iconColor="text-orange-600" iconBg="bg-orange-50" />
          <KpiCard label="Notices" value={String(notices.filter(n => !n.readBy.includes('u3')).length)} change="unread" icon={Bell} iconColor="text-blue-600" iconBg="bg-blue-50" />
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          {/* Recent Homework */}
          <Card className="p-4">
            <h3 className="text-sm font-semibold text-slate-800 mb-3">Recent Homework</h3>
            <div className="space-y-2">
              {childHomework.map(hw => (
                <div key={hw.id} className="flex items-start gap-3 rounded-lg border border-slate-100 p-2.5">
                  <div className={cn(
        ...
        {/* Latest Notices */}
        <Card className="p-4">
          <h3 className="text-sm font-semibold text-slate-800 mb-3">Latest Notices & Circulars</h3>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
            {notices.map(n => (
              <div key={n.id} className="flex items-start gap-3 rounded-lg border border-slate-100 p-3 hover:bg-slate-50">
                <div className={cn(
                  'flex h-8 w-8 shrink-0 items-center justify-center rounded-lg',
