    ...
    <div className="animate-fade-in">
      ...
      <div className="space-y-4 p-4 md:p-6">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
          <KpiCard label="Total Students" value={String(students.length)} change="+8 this term" trend="up" icon={Users} iconColor="text-blue-600" iconBg="bg-blue-50" />
          <KpiCard label="Total Staff" value={String(staff.length)} change="2 new hires" trend="up" icon={UserCheck} iconColor="text-indigo-600" iconBg="bg-indigo-50" />
          <KpiCard label="Attendance Today" value={`${attendancePct}%`} change={`${presentToday} present`} trend="up" icon={CalendarCheck} iconColor="text-violet-600" iconBg="bg-violet-50" />
          <KpiCard label="Fee Collected (Sep)" value={`₹${(collectedThisMonth / 100000).toFixed(1)}L`} change={`${Math.round((collectedThisMonth / target) * 100)}% of target`} trend="neutral" icon={CreditCard} iconColor="text-emerald-600" iconBg="bg-emerald-50" />
          <KpiCard label="Pending Enquiries" value={String(pendingEnquiries)} change="3 need follow-up" trend="down" icon={UserPlus} iconColor="text-blue-600" iconBg="bg-blue-50" />
          <KpiCard label="Buses Running" value={`${busesRunning}/3`} change="All on time" trend="up" icon={Bus} iconColor="text-teal-600" iconBg="bg-teal-50" />
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          {/* Attendance Trend */}
          <Card className="p-4">
            <h3 className="text-sm font-semibold text-slate-800 mb-1">Attendance Trend</h3>
          ...
          {/* Admission Funnel */}
          <Card className="p-4">
            <h3 className="text-sm font-semibold text-slate-800 mb-1">Admission Funnel</h3>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div className="space-y-4">
            <QuickActions>
              <QuickAction label="Add Student" icon={Plus} iconColor="text-blue-600" iconBg="bg-blue-50" />
              <QuickAction label="Send Notice" icon={Send} iconColor="text-pink-600" iconBg="bg-pink-50" />
              <QuickAction label="Collect Fee" icon={DollarSign} iconColor="text-emerald-600" iconBg="bg-emerald-50" />
              <QuickAction label="Mark Attendance" icon={CalendarCheck} iconColor="text-violet-600" iconBg="bg-violet-50" />
            </QuickActions>

            <Card className="p-4">
              <h3 className="text-sm font-semibold text-slate-800 mb-3">Latest Notices</h3>
              <div className="space-y-2">
